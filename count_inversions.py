...
def merge_sort_and_count_inversions(L, compare=operator.lt):
    if len(L) < 2:
        return L[:], 0
    else:
        middle_index = len(L)/ 2
        left, a = merge_sort_and_count_inversions(L[:middle_index], compare)
        right, b = merge_sort_and_count_inversions(L[middle_index:], compare)
        merged, c = merge_and_count_split_inv(left, right, compare)
        return merged, a+b+c
...
def testMergeAndCount():
    ...
    for L in Ls:
        sorted_list, num_inversions = merge_sort_and_count_inversions(L)
    return num_inversions

import timeit

t0 = timeit.Timer(testMergeAndCount)
print '1\n', 'MergeAndCount TestCases: 1 time\n', t0.timeit(number=1)
...
print "MergeAndCount IntegerArray.txt: 1 time\n", t1.timeit(number=1)

# def bruteForce(L, compare=operator.lt):
#     inversions = 0
#     for i in range(len(L)):
#         for j in range(len(L)):
#             if compare(i, j) and compare(L[i], L[j]):
#                 inversions += 1
#     return inversions
...
# def testBruteForceFile(fileName='IntegerArray.txt'):
#     inFile = open(fileName, "r")
#     intList = [int(line) for line in inFile]
